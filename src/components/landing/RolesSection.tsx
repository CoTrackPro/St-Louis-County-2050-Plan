...
function RoleCard({ role, billing }: { role: Role; billing: "monthly" | "yearly" }) {
        ...
        <div className="grid lg:grid-cols-12 gap-8 lg:gap-12 items-start">
          ...
          <div className="lg:col-span-7">
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                <h3 className={`text-2xl font-bold text-white ${role.accentColor}`}>
                  {role.title}
                </h3>
                {role.subRoles && (
                  <p className="text-xs text-gray-400 mt-0.5">
                    {role.subRoles.slice(0, 3).join(" · ")}
                    {role.subRoles.length > 3 ? " · more" : ""}
                  </p>
                )}
              </div>
            </div>
...
function BillingToggle({
  ...
  return (
    <div
      className="relative inline-flex bg-white/5 p-1.5 rounded-full border border-white/10 cursor-pointer select-none"
      role="group"
    ...
    >
      ...
      <div
        ...
        style={{ left: value === "monthly" ? "6px" : "50%", width: "calc(50% - 6px)" }}
        aria-hidden="true"
      />
      <button
        onClick={() => onChange("monthly")}
        className={`relative z-10 px-8 py-3 rounded-full text-sm font-bold transition-colors ${
          value === "monthly" ? "text-gray-900" : "text-gray-400 hover:text-white"
        }`}
      ...
      <button
        ...
        className={`relative z-10 px-8 py-3 rounded-full text-sm font-bold flex items-center gap-2 transition-colors ${
          value === "yearly" ? "text-gray-900" : "text-gray-400 hover:text-white"
        }`}
        aria-pressed={value === "yearly"}
      >
        Yearly
        <span
          className={`hidden sm:inline text-[10px] font-extrabold px-1.5 py-0.5 rounded ${
            value === "yearly" ? "bg-gray-900/20 text-gray-900" : "bg-[#f59e0b] text-gray-900"
          }`}
        >
          Save
        </span>
    ...
    <section id="choose-your-path" className="py-24 px-4 max-w-6xl mx-auto">
      <div className="mb-12 text-center">
        ...
        <p className="text-gray-400 max-w-xl mx-auto mb-10 text-lg leading-relaxed">
          Each membership is built around{" "}
          <strong className="text-white">Best Interests</strong>—what reduces harm and
          increases safety <em>right now</em>.
        </p>
      ...
      <div className="space-y-8">
        {ROLES.map((role) => (
          <RoleCard key={role.id} role={role} billing={billing} />
        ))}
      </div>
    </section>
  );
